...
def process_posts(inputs, outputs):
    ...
    posts_data = xml_parsed.select(xml['_Id'].alias('post_id'), xml['_AcceptedAnswerId'].alias('accepted_answer_id'), xml['_AnswerCount'].alias('answer_count'),
                 xml['_CreationDate'].alias('creation_date'), xml['_FavoriteCount'].alias('favorite_count'), xml['_OwnerUserId'].alias('owner_user_id'), 
                 xml['_ParentId'].alias('parent_id'), xml['_PostTypeId'].alias('post_type_id'), xml['_Title'].alias('title'), xml['_ViewCount'].alias('view_count'),
                 xml['_Tags'].alias('_Tags'))
